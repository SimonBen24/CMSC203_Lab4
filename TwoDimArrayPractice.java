...
public class TwoDimArrayPractice extends JFrame
{
   ...
   public void printArray() {
       for (int row = 0; row < intArray.length; row++) {
           for (int col = 0; col < intArray[row].length; col++) {
               System.out.print(intArray[row][col] + " "); // Print elements
               animate(row, col); // Animate the algorithm
           }
   ...
   */
   public void setValues(int value, int row) {
       if (row >= 0 && row < intArray.length) { // Check valid row index
           for (int col = 0; col < intArray[row].length; col++) {
               intArray[row][col] = value; // Set the specified value
               animate(row, col); // Animate the algorithm
           }
       } else {
   ...
   */
   public int findMinimum(int column) {
       if (column >= 0 && column < intArray[0].length) { // Check valid column index
           int min = intArray[0][column];
           for (int row = 1; row < intArray.length; row++) {
               if (intArray[row][column] < min) {
                   min = intArray[row][column]; // Update minimum
               }
               animate(row, column); // Animate the algorithm
           }
           return min; // Return the minimum value
       } else {
           System.out.println("Invalid column index.");
           return -1; // Return -1 for invalid column index
       }
   }
       ...
       for (int row = 0; row < intArray.length; row++) {
           for (int col = 0; col < intArray[row].length; col++) {
               if (intArray[row][col] == value) {
                   count++; // Increment count for matching value
               }
               animate(row, col); // Animate the algorithm
           }
       }
